...
public:
   ...
   Error_code range_query_by_Hamming_dist_knn(const Leaf_entry& query_data, int range, Leaf_entry* query_results, int &number_of_query_results, int &number_of_io,int &number_of_dist_computation);
...
Error_code ND_tree::range_query_by_Hamming_dist_knn(const Leaf_entry& query_data, int range, Leaf_entry* query_results, int &number_of_query_results, int &number_of_io,int &number_of_dist_computation){
   int i;
   Error_code result;
   ...
   if(height == 1){ // root is a leaf
      ...
      root_node->read_node(ND_file, root);
      number_of_io++;
      Error_code result = root_node->retrieve_by_hamming_dist_knn(query_data, range, query_results, number_of_query_results, number_of_io, number_of_dist_computation);
      delete root_node;
      return result;
   }else{
      stack<unsigned int> dir_check_list; // dir_check_list contains a list of block number of dir nodes that may contain the query point
      ...
      while(!dir_check_list.empty()){
         ...
         if(tmp_height == 2) // One level higher than leaf
            for(i = 0; i < block_numbers_in_range.size(); i++){ // go through all the relevant entries
               ...
               number_of_io++;
               result = l_node->retrieve_by_hamming_dist_knn(query_data, range, query_results, number_of_query_results, number_of_io,number_of_dist_computation);
            }
         else{
            assert(tmp_height > 2);
            for(i = 0; i < block_numbers_in_range.size(); i++){ // go through all the relevant entries
