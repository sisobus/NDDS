...
public:
   ...
   Error_code range_query_by_Hamming_dist(const Leaf_entry& query_data, int range, Leaf_entry* query_results, int &number_of_query_results, int &number_of_io);
...
Error_code ND_tree::range_query_by_Hamming_dist(const Leaf_entry& query_data, int range, Leaf_entry* query_results, int &number_of_query_results, int &number_of_io){
   int i;
   Error_code result;
   ...
   if(height == 1){ // root is a leaf
      ...
      root_node->read_node(ND_file, root);
      number_of_io++;
      Error_code result = root_node->retrieve_by_hamming_dist(query_data, range, query_results, number_of_query_results, number_of_io);
      delete root_node;
      return result;
   }else{
      stack<unsigned int> dir_check_list; // dir_check_list contains a list of block number of dir nodes that may contain the query point
      ...
      while(!dir_check_list.empty()){
         ...
         d_node->find_entries_by_hamming_dist(query_data, range, block_numbers_in_range);
         ...
         if(tmp_height == 2) // One level higher than leaf
            for(i = 0; i < block_numbers_in_range.size(); i++){ // go through all the relevant entries
               ...
               number_of_io++;
               result = l_node->retrieve_by_hamming_dist(query_data, range, query_results, number_of_query_results, number_of_io);
            }
         else{
            assert(tmp_height > 2);
            for(i = 0; i < block_numbers_in_range.size(); i++){ // go through all the relevant entries
