...
MVPTree* mvptree_alloc(MVPTree *tree, CmpFunc distance,unsigned int bf,unsigned int p,\
    ...
    // For some reason, the original code used give an error
    // so Alok changed it
    //retTree->datatype     = 0;
    retTree->datatype     = UINT32ARRAY;
    retTree->node         = NULL;
    retTree->fd           = 0;
    retTree->k            = 0;
    retTree->size         = 0;
...
static Node* create_internal(unsigned int bf){
    ...
    // g++ wont accept this statement. Hence Alok changed it.
    node->internal.child_nodes = (void **)calloc(bf*bf,sizeof(Node*));
...
static void _mvptree_clear(MVPTree *tree, Node *node, MVPFreeFunc free_func, int lvl){
	...
	int i, fanout = (tree->branchfactor)*(tree->branchfactor);
	for (i = 0;i < fanout;i++){
	    _mvptree_clear(tree, (Node *)node->internal.child_nodes[i], free_func, lvl+1);
	}
...
//        int errCode = select_vantage_points(points, nbpoints, &sv1_pos, &sv2_pos, tree->dist); 
        if (select_vantage_points(points, nbpoints, &sv1_pos, &sv2_pos, tree->dist) < 0){
//	    if (errCode<0){
		...
		*error = MVP_NOSORT;
		...
		for (j=0;j<tree->branchfactor;j++){
		    ...
		    child = _mvptree_add(tree,\
                                  (Node*)new_node->internal.child_nodes[i*tree->branchfactor + j],\
...
static off_t write_datapoint(MVPDP *dp, MVPTree *tree){
    ...
    active = 1;
    // Alok commented this because he changed id from
    // a string to a number
//    uint8_t idlen = strlen(dp->id);
    uint8_t idlen = sizeof(dp->id);
...
static off_t _mvptree_write(MVPTree *tree,Node *node,MVPError *error,int lvl){
	...
	for (i=0;i<fanout;i++){
	    ...
	    off_t offset = _mvptree_write(tree, (Node*)node->internal.child_nodes[i], error, lvl+2);
...
static MVPDP* read_datapoint(MVPTree *tree)
{
    ...
    assert(idlen == sizeof(dp->id));
    // Alok commented this as he changed type of id from
    // a string to a number
...
static MVPError _mvptree_print(FILE *stream, MVPTree *tree, Node *node, int lvl){
	...
	if (next_node->leaf.type == LEAF_NODE){
	    ...
	    fprintf(stream,"\n");
	    for (i=0;i<fanout;i++){
		error = _mvptree_print(stream, tree, (Node*)node->internal.child_nodes[i], lvl+2);
		if (error != MVP_SUCCESS) break;
	    }
	} else {
	    error = MVP_UNRECOGNIZED;
