...
public:
    ...
    Error_code retrieve_by_hamming_dist_knn(const Leaf_entry & query_data, int range, Leaf_entry* query_results, int& number_of_query_results,int &number_of_io,int &number_of_dist_computation,double & r,priority_queue<double>& knns,vector<map<char,double> >& freq);
...
Error_code Leaf_node::retrieve_by_hamming_dist_knn(const Leaf_entry& query_data, int range, Leaf_entry * query_results, int& number_of_query_results,int& number_of_io,int &number_of_dist_computation,double& r,priority_queue<double>& knns,vector<map<char,double> >&freq)
{
    int i, j;
    int orig_count = number_of_query_results;
    int totalEntrieNum=0;
    ...
    for(i = 0; i < count; i++)
    {
        ...
        for(j = 0; j < DIM && cur_dist <= r; j++){
            if(entries[i].key[j] != query_data.key[j])  {
                cur_dist += 1;
            } else {
                cur_dist += freq[j][query_data.key[j]]/DIM;
            }
        }
        if(cur_dist <= r)
        { // current vector within range
            //number_of_dist_computation += number_of_query_results;
            if ( (int)knns.size() >= range ) {
                double t = knns.top();knns.pop();
                r = t;
                knns.push(cur_dist);
            } else {
                knns.push(cur_dist);
