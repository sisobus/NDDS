...
vector<map<char,double> > batchBuild_with_duplicate(int size)
{
		logO.clearLogs();
	...
	string line;
    vector<map<char,double> > freq(DIM);
...
debug_dataIndex = i;
		...
		for(int j = 0; j < DIM; j++)
		{
			instr >> n;
			new_data.key[j] = n;
            if ( freq[j].count(new_data.key[j]) == 0 ) {
                freq[j][new_data.key[j]] = 1;
            } else {
                freq[j][new_data.key[j]] += 1;
            }
		}
        for ( int j = 0 ; j < DIM ; j++ ) 
            for ( map<char,double>::iterator it=freq[j].begin();it!=freq[j].end();it++ )
                it->second = 1.0 - (it->second/ num_of_points);
		//new_data.record = i;
		new_data.record = 1; 
...
void batchKnnQuery(vector<map<char,double> >& freq	)
{
	...
	for(int range=1;range<=RANGE_STOP_BEFORE;range+=RANGE_SIZE_STEP)
	{
		...
		for(int i = 0; i < num_of_points; i++)
		{
            ...
            priority_queue<double> knns;

			result = ndt.range_query_by_Hamming_dist_knn(query_data, range, query_results, query_results_size, number_of_io, number_of_dist_computation, knns,freq);
		...
		cout << "For k: "<<range<<" range query I/O: " << static_cast<double>(total_number_of_io) / num_of_points << endl;
...
//assert(1==0);
    ...
    vector<map<char,double> > freq;
	freq = batchBuild_with_duplicate(MAX_LINE_IN_SOURCE_FILE);
	//batchBoxQuery();
	batchKnnQuery(freq);
